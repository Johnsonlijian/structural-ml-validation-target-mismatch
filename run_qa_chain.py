...
One-shot QA chain for Cursor / local terminal:
  ...
  5) Run R02/R04 hardening diagnostics (Tables S11-S20).
  6) Optionally run R05 retraining diagnostics (Tables S21-S26).
  7) Optionally run R06 shift/proxy diagnostics (Tables S27-S28).

Use the same interpreter you use for other scripts, e.g.::

    python code/run_qa_chain.py

Optional flags: ``--skip-06``, ``--skip-24``, ``--skip-33``, ``--skip-34``,
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run 06 + 24 + normalize results.csv + run 33 + run 34 + diagnostics.")
